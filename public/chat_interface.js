...
async function createMessage(type = "user", text = "") {
    setTimeout(
        async () => {
            // Styles from config
            const config_style = await fetch(
                `${env.ip}:${env.port}/config/styles`,
            ).then((res) => res.json());
            ...
            profile_picture_section.classList.add(
                `section-${type}-profile-picture`,
            );
...
function streamingMessage(chunk) {
    ...
    const text_sections = document.querySelectorAll(
        ".section-gpt-message-text",
    );
...
function handleRestart(e) {
    ...
    document.cookie =
        "CUC-ID" + "=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
...
function displayError(error) {
    ...
    if (!error_box.classList.contains("opacity"))
        error_box.classList.add("opacity");

    // Remove display: none
    if (error_box.classList.contains("none"))
        error_box.classList.remove("none");

    // Remove appearance animation after it played
    setTimeout(
        () =>
            error_box.classList.contains("opacity")
                ? error_box.classList.remove("opacity")
                : null,
        300,
    );
    ...
    setTimeout(() => {
        ...
        if (!error_box.classList.contains("opacity-reverse"))
            error_box.classList.add("opacity-reverse");

        setTimeout(() => {
            // Remove disappearance animation
            if (error_box.classList.contains("opacity-reverse"))
                error_box.classList.remove("opacity-reverse");

            // Add display: none
            if (!error_box.classList.contains("none"))
                error_box.classList.add("none");
...
socket.on("fully_received", (data) => {
    ...
    for (let i = 0; codeMessages.length > i; i++) {
        ...
        if (
            !codeMessages[i].classList.contains("hljs") &&
            codeMessages[i].getAttribute("class")
        ) {
