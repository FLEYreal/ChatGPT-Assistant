...
async function check_api_file(config, source = 'config.js', message = 'This file is important for the script!') {
    try {
        ...
        if (config.locations.api) {
            ...
            logging.warn(`File \"${source}\" doesn't exist!\n         This file is important for API!\n         If you lost the file, you can restore it by either downloading a copy from our official sources (StormShop) or contacting us personally: borisov.nikit.off@gmail.com\n`);
        }
    }
}
...
async function check() {
    ...
    if (config) {
        if (!config.locations) logging.exitWithError(`"locations" doesn't exist in config file!\n         It defines in what locations GPT will be working!\n`);
        if (!config.gpt_version) logging.exitWithError(`"gpt_version" doesn't exist in config file!\n         It defiens what version of chatGPT to use\n         See more about GPT versions here: https://platform.openai.com/docs/models!\n`);
        if (!config.max_tokens) logging.exitWithError(`"max_tokens" doesn\'t exist in config file!\n         It defines the limits amount of tokens per chatGPT message!\n`);
        if (!config.instructions) logging.exitWithError(`"instructions" doesn\'t exist in config file!\n         It gives chatGPT instructions on what to say and what to do!\n`);
        if (!config.delete_restarted_conversations) logging.exitWithError(`"delete_restarted_conversations" doesn\'t exist in config file!\n         It defines either to delete restarted (by button) conversation or not!\n`);
        if (!config.contact_email) logging.exitWithError(`"contact_email" doesn\'t exist in config file!\n         It defines a public email people will see to contact you!\n`);
        if (config.locations.discord && !config.discord) logging.exitWithError(`"discord" doesn\'t exist in config file!\n         It defines all important params related to discord bot!\n`);
        if (config.locations.telegram && !config.telegram) logging.exitWithError(`"telegram" doesn\'t exist in config file!\n         It defines all important params related to telegram bot!\n`);
    }
}
